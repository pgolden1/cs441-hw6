...
@implementation YourSSVc

- (void)viewDidLoad {
    [super viewDidLoad];
    NSLog(@"%@", self.username);
    self.sessionInfo = [[NSMutableArray alloc] initWithCapacity:8];
    self.sessionIDs = [[NSMutableArray alloc] initWithCapacity:8];
}

-(void)viewDidAppear:(BOOL)animated {
    [self getRSVPedSessions];
}


- (void)getRSVPedSessions {
    NSLog(@"in getRSVPedSessions");
    [self.sessionInfo removeAllObjects];
    NSString *post = [NSString stringWithFormat:@"username=%@", self.username, nil];
    NSData *postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
    
    NSString *postLength = [NSString stringWithFormat:@"%lu", [postData length]];
    
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL: [NSURL URLWithString: @"https://cs.binghamton.edu/~jkunnum1/php/active_rsvps.php"]] ;
...
-(void)updateDisplay:(NSString *)data {
    [self.sessionInfo addObject:data];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [self.sessionInfo objectAtIndex:indexPath.row];
    
    // make button to add
    CGRect buttonRect = CGRectMake(310, 15, 40, 25);
...
- (void)myaction:(UIButton *)sender {
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL: [NSURL URLWithString: @"https://cs.binghamton.edu/~jkunnum1/php/unrsvp.php"]] ;
    [request setHTTPMethod:@"POST"];
    [request setValue:postLength forHTTPHeaderField:@"Content-Length"];
    [request setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
    [request setHTTPBody:postData];
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if(httpResponse.statusCode == 200)
        {
            ...
            {
                NSLog(@"Login SUCCESS");
                NSString *message = @"Successfully un-RSVPed!";
            ...
            else
            {
                ...
                switch (error_num) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        message = @"You have already un-RSVPed!";
                        break;
                    case 4:
...
-(void)showMessage:(NSString *)message {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"un-RSVP"
                                                                             message:message
